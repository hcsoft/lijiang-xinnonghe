					...
					data-options="iconCls:'icon-edit',singleSelect:true" rownumbers="true"> 
						<thead>
							<tr>
								...
								<th data-options="field:'belongtocountry',width:80,align:'center',formatter:format,editor:{type:'combobox',options:{required:true,editable:false,valueField:'key',textField: 'value',data:locationdata}}">所属乡镇</th>
								<th data-options="field:'belongtowns',width:100,align:'center',formatter:format,editor:{type:'combobox',options:{required:true,editable:false,valueField:'key',textField: 'value',data:locationdata}}">坐落位置</th>
								<th data-options="field:'detailaddress',width:120,align:'center',editor:{type:'text',options:{required:false}}">详细地址</th>
								<th data-options="field:'areaplough',width:60,align:'center',editor:{type:'numberbox',options:{precision:2,min:0,required:true}}">农用地面积(㎡)</th>
								<th data-options="field:'areabuild',width:60,align:'center',editor:{type:'numberbox',options:{precision:2,min:0,required:true}}">建设用地面积(㎡)</th>
								<th data-options="field:'areauseless',width:60,align:'center',editor:{type:'numberbox',options:{precision:2,min:0,required:true}}">未利用地面积(㎡)</th>
								<th data-options="field:'areatotal',width:60,align:'center',readOnly:'true',editor:{type:'numberbox',options:{precision:2,min:0,required:true}}">合计(㎡)</th>
							</tr>
						</thead>
					</table>
		</div>
