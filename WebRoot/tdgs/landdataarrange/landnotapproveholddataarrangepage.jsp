...
<head>
	...
	$(function(){
			...
			var compareDataAry = [new CompareData(true,' and sourcedetailaddress = detailaddress ','按地址','comparenotes2','地址(比对情况)'),
		...
		function exportExcel(){
			...
			params['excelName'] = '未批先占数据比对结果.xls';	
		...
		function exportMatchResultExcel(){
			...
			params['displayNames'] = '状态,计算机编码(未批先占),纳税人名称(未批先占),主管机关,税收管理员,宗地编号(未批先占),详细地址(未批先占),土地证号(未批先占),' +
			'面积(未批先占),获取日期(未批先占),计算机编码(采集),纳税人名称(采集),宗地编号(采集),详细地址(采集),土地证号(采集),面积(采集),获取日期(采集)'+exportvalue;
			params['excelName'] = '未批先占数据比对结果.xls';	
		...
		function exit(){
			...
			var currentUrl = "/tdgs/landdataarrange/landnotapproveholddataarrangepage.jsp";
		...
		function getCompareMsg(sourceRow,estateRow){
			...
			if(sourceRow.taxpayerid == estateRow.taxpayerid){
				showInfo += "未批先占计算机编码:"+sourceRow.taxpayerid+"，采集计算编码："+estateRow.taxpayerid+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>未批先占计算机编码:"+sourceRow.taxpayerid+"，采集计算编码："+estateRow.taxpayerid+"</span><br/>";
			}
			//taxpayername
			if(sourceRow.taxpayername == estateRow.taxpayername){
				showInfo += "未批先占纳税人名称:"+sourceRow.taxpayername+"，采集纳税人名称："+estateRow.taxpayername+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>未批先占纳税人名称:"+sourceRow.taxpayername+"，采集纳税人名称："+estateRow.taxpayername+"</span><br/>";
			}
			/*
			//estateserial
			if(sourceRow.estateserial == estateRow.estateserial){
				showInfo += "未批先占宗地编号:"+sourceRow.estateserial+"，采集土地编码："+estateRow.estateserial+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>未批先占宗地编号:"+sourceRow.estateserial+"，采集土地编码："+estateRow.estateserial+"</span><br/>";
			}
			*/
			//area
			if(sourceRow.areatotal == estateRow.landarea){
				showInfo += "未批先占面积:"+sourceRow.areatotal+"，采集面积："+estateRow.landarea+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>未批先占面积:"+sourceRow.areatotal+"，采集面积："+estateRow.landarea+"</span><br/>";
			}
			//date
			...
			if(sourceRow.gaindate == estateRow.holddate){
				showInfo += "未批先占日期:"+gdate+"，采集日期："+hdate+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>未批先占日期:"+gdate+"，采集日期："+hdate+"</span><br/>";
			}
			//detailaddress
			if(sourceRow.detailaddress == estateRow.detailaddress){
				showInfo += "未批先占详细地址:"+sourceRow.detailaddress+"，采集详细地址："+estateRow.detailaddress+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>未批先占详细地址:"+sourceRow.detailaddress+"，采集详细地址："+estateRow.detailaddress+"</span><br/>";
			}
			return showInfo;
		}
		...
		function undoAllCheckMatch(){
			...
			if(matchId == ''){
			    ...
			    $.ajax(
			   {
				   ...
				   data:{'landsourceid':matchId,'bustype':1},
				   async:false,
				   dataType:'json',
				   success:function(jsondata){
					   if(jsondata.sucess){
...
<body>
	...
	<div id="querywindow" class="easyui-window" data-options="closed:true,modal:true,title:'土地未批先占数据整理查询条件',collapsible:false,minimizable:false,maximizable:false,closable:true" style="width:640px;height:310px;">
