...
<head>
	...
	$(function(){
			...
			var compareDataAry = [new CompareData(true,' and sourcelandcertificate= landcertificate ','按土地证号编号','comparenotes1','土地证号(比对情况)'),
		...
		function exportExcel(){
			var params = getCondition();
			params['propertyNames'] = 'stateName,taxpayerid,taxpayername,importtaxpayername,landcertificate,estateserial,area,providedate';
			params['displayNames'] = '状态,计算机编码,纳税人名称,导入纳税人名称,土地证号,宗地编号,土地面积,发放日期';
			params['excelName'] = '地籍数据比对结果.xls';	
		...
		function exportMatchResultExcel(){
			...
			params['displayNames'] = '状态,计算机编码(地籍),纳税人名称(地籍),主管机关,税收管理员,宗地编号(地籍),详细地址(地籍),土地证号(地籍),' +
			'面积(地籍),获取日期(地籍),计算机编码(采集),纳税人名称(采集),宗地编号(采集),详细地址(采集),土地证号(采集),面积(采集),获取日期(采集)'+exportvalue;
			params['excelName'] = '地籍数据比对结果.xls';	
		...
		function queryCompareInfo(){
			...
			opts.url = '/landdataarange/comparetaxpayerinfo.do?d='+new Date();
		...
		function exit(){
			...
			var currentUrl = "/tdgs/landdataarrange/landcadastraldataarrangepage.jsp";
		...
		function getCompareMsg(sourceRow,estateRow){
			...
			if(sourceRow.taxpayerid == estateRow.taxpayerid){
				showInfo += "地籍计算机编码:"+sourceRow.taxpayerid+"，采集计算编码："+estateRow.taxpayerid+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>地籍计算机编码:"+sourceRow.taxpayerid+"，采集计算编码："+estateRow.taxpayerid+"</span><br/>";
			}
			//taxpayername
			if(sourceRow.taxpayername == estateRow.taxpayername){
				showInfo += "地籍纳税人名称:"+sourceRow.taxpayername+"，采集纳税人名称："+estateRow.taxpayername+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>地籍纳税人名称:"+sourceRow.taxpayername+"，采集纳税人名称："+estateRow.taxpayername+"</span><br/>";
			}
			//landcertificate
			if(sourceRow.landcertificate == estateRow.landcertificate){
				showInfo += "地籍土地证号:"+sourceRow.landcertificate+"，采集土地证号："+estateRow.landcertificate+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>地籍土地证号:"+sourceRow.landcertificate+"，采集土地证号："+estateRow.landcertificate+"</span><br/>";
			}
			//area
			if(sourceRow.area == estateRow.landarea){
				showInfo += "地籍面积:"+sourceRow.area+"，采集面积："+estateRow.landarea+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>地籍面积:"+sourceRow.area+"，采集面积："+estateRow.landarea+"</span><br/>";
			}
			//date
			var gdate = formatDatebox(sourceRow.providedate);
			var hdate = formatDatebox(estateRow.holddate);
			if(sourceRow.gaindate == estateRow.holddate){
				showInfo += "地籍日期:"+gdate+"，采集日期："+hdate+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>地籍日期:"+gdate+"，采集日期："+hdate+"</span><br/>";
			}
			//detailaddress
			if(sourceRow.address == estateRow.detailaddress){
				showInfo += "地籍详细地址:"+sourceRow.address+"，采集详细地址："+estateRow.detailaddress+"<br/>";
			}else{
				showInfo += "<span style='color:red;'>地籍详细地址:"+sourceRow.address+"，采集详细地址："+estateRow.detailaddress+"</span><br/>";
			}
			return showInfo;
		}
		function matchCompare(index){
			...
			if(sourceRow && estateRow){
				...
				$.messager.confirm('确认需要匹配吗？', showInfo, function(r){
					if (r){
						var landgainid = sourceRow.estateid;
		...
		function undoAllCheckMatch(){
			...
			if(matchId == ''){
			    ...
			    $.ajax(
			   {
				   ...
				   data:{'landsourceid':matchId,'bustype':2},
				   async:false,
				   dataType:'json',
				   success:function(jsondata){
					   if(jsondata.sucess){
		...
		function addCheckBox(value,row,index){
			var result = "";
			if(row.state == 4){   //完全匹配
				 result = "<input type='checkbox' value='"+row.estateid+"' />";
			}
			return result;
		}
		var checkedCb = false;
			...
			<table id='landdataarrangegrid' class="easyui-datagrid" style="height:500px;overflow: scroll;"
				...
				<thead>
					<tr>
						...
						<th data-options="field:'estateid',hidden:true,width:100,align:'center',editor:{type:'validatebox'}">主键</th>
						...
						<th data-options="field:'landcertificate',width:230,align:'center',editor:{type:'validatebox'}">土地证号</th>
						<th data-options="field:'estateserial',width:120,width:120,align:'center',editor:{type:'validatebox'}">宗地编号</th>
						<th data-options="field:'area',width:100,align:'center',editor:{type:'validatebox'}">土地面积</th>
						<th data-options="field:'providedate',width:80,align:'center',formatter:formatterDate,editor:{type:'validatebox'}">发放日期</th>
					</tr>
				</thead>
			</table>
	    </form>
					...
					data-options="iconCls:'icon-edit',singleSelect:true,idField:'taxpayerid',onLoadSuccess:function(data){
						...
						<thead>
							<tr>
								...
								<th data-options="field:'taxpayername',width:230,align:'left',editor:{type:'validatebox'}">纳税人名称</th>
								<th data-options="field:'area',width:100,align:'center',editor:{type:'validatebox'}">出让面积</th>
								...
								<th data-options="field:'providedate',width:80,align:'center',formatter:formatterDate,editor:{type:'validatebox'}">出让日期</th>
								<th data-options="field:'address',width:230,align:'left',editor:{type:'validatebox'}">出让土地详细地址</th>
							</tr>
						</thead>
					</table>
	     </div>
					...
					data-options="iconCls:'icon-edit',singleSelect:true,idField:'taxpayerid',onLoadSuccess:function(data){
						...
						<thead>
							<tr>
								...
								<th data-options="field:'detailaddress',width:230,align:'left',editor:{type:'validatebox'}">土地详细地址</th>
								<th data-options="field:'remark',hidden:true,width:120,align:'center'">备注</th>
							</tr>
						</thead>
					</table>
	     </div>
