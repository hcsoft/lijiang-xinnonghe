...
<script>
  ...
  <body class="claro" style="overflow:hidden" >
    <div class="easyui-layout" style="width:100%;height:650px;" id="layoutDiv">
		...
		<div data-options="region:'east'" style="width:300px;height:615px;padding:0px;">
			<div style="width:450px;height:500px;padding:20px">
			    <span id="toc"></span>
		    </div>
		</div>
    </div>
...
=======
		...
		<div id="layerInfoConfigWindow" class="easyui-dialog" title="图层控制" data-options="iconCls:'icon-search',closed:true,modal:false,collapsible:true"  style="width:450px;height:500px;padding:20px">
			<span id="toc"></span>
		</div>
		...
		<div id="taxinfo" class="easyui-dialog" title="&nbsp;" data-options="iconCls:'icon-save',closed:true,modal:false,collapsible:true"  style="width:740px;height:540px;">
			...
			<div id="taxinfoLayout" class="easyui-layout" style="width:700px;height:500px;" data-options="fit:true">
				<div data-options="region:'center',split:true" title="">
					<ul id="taxinfoTree" data-options="animate:true"></ul>
				</div>
				<div data-options="region:'east',title:'纳税人信息',iconCls:'icon-ok'" style="width:450px;">

					<div id="tb" style="padding:5px;height:auto">
						<div>
							纳税人名称: <input id="taxpayernameFilterId" class="easyui-validatebox" style="width:80px" data-options="required:false">&nbsp;&nbsp;&nbsp;&nbsp;
							<a href="javascript:filterTaxpayer()" class="easyui-linkbutton" iconCls="icon-search">查询</a>&nbsp;&nbsp;&nbsp;&nbsp;
							<a href="javascript:selectTaxpayer()" class="easyui-linkbutton" iconCls="icon-add">选择</a>&nbsp;&nbsp;&nbsp;&nbsp;
						</div>
					</div>


					<table id="taxinfoList" class="easyui-datagrid"
							data-options="rownumbers:true,sortable:true,singleSelect:true,pagination:true,border:false,fit:true,fitColumns:true,toolbar:'#tb',
											rowStyler: function(index,row){
												if (row.estateseriallayer != null && $.trim(row.estateseriallayer) != ''){
						...
						<thead>
							<tr>
								...
								<th data-options="field:'landarea',align:'right'" width="70">土地面积</th>
							</tr>
						</thead>
					</table>
						<script type="text/javascript">
							function filterTaxpayer(){
								var params = {};
								params['belongtowns']=belongtownsNodeId;
								params['taxpayername'] = $('#taxpayernameFilterId').val();
								var opts = $('#taxinfoList').datagrid('options');
								opts.url = '/Relation/getTaxinfoListByBelongtowns.do?d='+new Date();
								$('#taxinfoList').datagrid('load',params); 
							}
							...
							function selectTaxpayer(){
									...
									$('#positionDiv').window('setTitle','你当前选择是:' + "");
							...
							function selectTaxpayer2(){
									
									var payerInfoList = $('#payerInfoId').datagrid('getChecked');
									//alert(payerInfoList[0].estateid);
									if(payerInfoList.length <= 0){
										$.messager.alert('提示','请先选择需要关联的记录!');
										return ;
									}
									$('#positionDiv').window('setTitle','你当前选择是:' + payerInfoList[0].taxpayername);
									$('#taxinfo').window('setTitle','你当前选择是:' + "");
									$('#positionDiv').panel('collapse');
									estateid = payerInfoList[0].estateid;	
									//alert(estateid);
							}
		...
		<div id="positionDiv" class="easyui-dialog" title="纳税人查询" data-options="iconCls:'icon-search',closed:true,modal:false,collapsible:true"  style="width:550px;padding:5px">
			...
			<a href="javascript:selectTaxpayer2()" class="easyui-linkbutton" iconCls="icon-add">选择</a>&nbsp;&nbsp;
			<table id="payerInfoId"  class="easyui-datagrid" style="width:510px;height:150px;padding:5px"
					data-options="singleSelect:true,fitColumns:true,collapsible:true,
>>>>>>> 42d456c6a379ade7e5f1d64f035601476e61e37d
											rowStyler: function(index,row){
...
=======
		<div id="taxinfoEdit" class="easyui-dialog" title="税源详细信息" data-options="iconCls:'icon-business3',closed:true,modal:false,collapsible:true"  style="width:540px;height:240px;">
					<table id="taxinfoEditList" class="easyui-datagrid"
							data-options="rownumbers:true,sortable:true,singleSelect:true,pagination:true,border:false,fit:true,fitColumns:true,toolbar:toolbar2">
						<thead>
							<tr>
								<th data-options="field:'ck',checkbox:true"></th>
								<th data-options="field:'taxpayername'" width="180">单位名称</th>
								<th data-options="field:'detailaddress'" width="180">地址</th>
								<th data-options="field:'landarea',align:'right'" width="100">土地面积</th>
								<th data-options="field:'areaofstructure',align:'right'" width="100">房产面积</th>
							</tr>
						</thead>
					</table>
						<script type="text/javascript">
											...
											$.ajax({
												   ...
												   success: function(jsondata){
														if(jsondata == "02"){
															...
															var params = {};
															params['objectid'] = clickObjectid;
														    var opts = $('#taxinfoEditList').datagrid('options');
														    opts.url = 'Relation/getTaxinfoListByObjectid.do?d='+new Date();
														    $('#taxinfoEditList').datagrid('load',params); 	
													    
															$('#taxinfoTree').tree('options').url = "/TreeServlet?pid=530000&levels=1";
															$('#taxinfoTree').tree('reload'); 
														}
														if(jsondata == "01"){  //删除税源，刷新
															$.messager.alert('提示','解除成功!');
															$('#taxinfoEdit').dialog('close');	
															featureLayer3.refresh();
															featureLayer2.refresh();
													    ...
													    var opts = $('#payerInfoId').datagrid('options');
													    opts.url = '/Relation/queryTaxpayerInfo.do?d='+new Date();
													    $('#payerInfoId').datagrid('load',params); 	
												   }
											});
										}
									});
		...
		<div id="positioninfo" class="easyui-dialog" title="&nbsp;" data-options="iconCls:'icon-business5',closed:true,modal:false,collapsible:true"  style="width:300px;height:500px;">
