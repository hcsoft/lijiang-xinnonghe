...
<head>
	...
	var menu24 = [{
					...
					iconCls:'icon-search',
					handler:function(){
						$('#groundhirequerywindow').window('open');
					}
				},{
					id:'showdetail',
					text:'明细查看',
					iconCls:'icon-tip',
					handler:function(){
						var tdxxrow = $('#groundhiregrid').datagrid('getSelected');//获取土地信息选中行
						if(tdxxrow){
					...
					handler:function(){
						var row = $('#groundhiregrid').datagrid('getSelected');
						if(row){
							businesstype = '24';
							...
							$.messager.alert('返回消息',"请选择需要融资租赁的土地！");
						}
					}
				}];
	var menu25 = [{
					...
					handler:function(){
						...
						if(row){
							...
							$.messager.alert('返回消息',"请选择需要出典的土地！");
						}
					}
				}];
	var menu31 = [{
					...
					handler:function(){
						...
						if(row){
							...
							$.messager.alert('返回消息',"请选择需要出租的土地！");
						}
					}
				}];
	...
	$(function(){
		...
		//alert(businesstype);
		if(businesstype == '' ){
			businesstype ="'24','25','31'";
			$('#groundhiregrid').datagrid({toolbar:menu24});
		}
		if(businesstype == '24' ){
			$('#groundhiregrid').datagrid({toolbar:menu24});
		}
		if(businesstype == '25' ){
			$('#groundhiregrid').datagrid({toolbar:menu25});
		}
		if(businesstype == '31' ){
			$('#groundhiregrid').datagrid({toolbar:menu31});
		}
		$.ajax({
			   type: "post",
				async:false,
	   ...
	   $.ajax({
			...
			$('#groundcheckgrid').datagrid({
					...
					toolbar:[{
						...
						handler:function(){
							...
							if(row){
								...
								if(row.businesscode =='21' || row.businesscode =='22' || row.businesscode =='23' || row.businesscode =='26'){//转让
									...
									taxpayername = row.lesseestaxpayername;
									$('#groundbusinesswindow').window('open');
									$('#groundbusinesswindow').window('refresh', '../groundhire/reginfo.jsp');
								}
								if(row.businesscode =='24' || row.businesscode =='25' || row.businesscode =='31'){//出租
									targetestateid = row.targetestateid;
									businesstype = row.businesscode;
									busid = row.busid;
									estateid = row.estateid;
						...
						iconCls:'icon-search',
						handler:function(){
							$('#groundhirewindow').window('open');
							$('#groundhiregrid').datagrid('loadData',{total:0,rows:[]});
						}
					},{
						text:'终审',
						iconCls:'icon-check',
			...
			var p = $('#groundhiregrid').datagrid('getPager');  
				$(p).pagination({  
					showPageList:false
				});
		...
		function query(){
			var params = {};
			var fields =$('#groundhirequeryform').serializeArray();
			$.each( fields, function(i, field){
				params[field.name] = field.value;
			}); 
			$('#groundhiregrid').datagrid('loadData',{total:0,rows:[]});
			var opts = $('#groundhiregrid').datagrid('options');
			opts.url = '/GroundAssignmentServlet/getgroundinfo.do';
			$('#groundhiregrid').datagrid('load',params); 
			var p = $('#groundhiregrid').datagrid('getPager');  
			$(p).pagination({   
				showPageList:false,
				pageSize: 15
			});
			$('#groundhirequerywindow').window('close');
...
<body>
	<form id="groundhireform" method="post">
					...
					data-options="iconCls:'icon-edit',singleSelect:true" rownumbers="true"> 
						<thead>
							<tr>
								...
								<th data-options="field:'lessorid',width:100,align:'center',editor:{type:'validatebox'}">出典方计算机编码</th>
								<th data-options="field:'lessortaxpayername',width:200,align:'center',editor:{type:'validatebox'}">出典方名称</th>
								<th data-options="field:'lesseesid',width:100,align:'center',editor:{type:'validatebox'}">承典方计算机编码</th>
								<th data-options="field:'lesseestaxpayername',width:200,align:'center',editor:{type:'validatebox'}">承典方名称</th>
								...
								<th data-options="field:'transmoney',width:60,align:'center',editor:{type:'validatebox'}">年租金</th>
								<th data-options="field:'limitbegins',width:60,align:'center',editor:{type:'validatebox'}">出典起时间</th>
								<th data-options="field:'limitends',width:60,align:'center',editor:{type:'validatebox'}">出典止时间</th>
	...
	<div id="groundhirequerywindow" class="easyui-window" data-options="closed:true,modal:true,title:'土地信息查询',collapsible:false,minimizable:false,maximizable:false,closable:true" style="width:940px;height:300px;">
		<div class="easyui-panel" title="" style="width:900px;">
			<form id="groundhirequeryform" method="post">
	...
	<div id="groundcheckquerywindow" class="easyui-window" data-options="closed:true,modal:true,title:'土地出典查询',collapsible:false,minimizable:false,maximizable:false,closable:true" style="width:940px;height:300px;">
		<div class="easyui-panel" title="" style="width:900px;">
	...
	<div id="groundhirewindow" class="easyui-window" data-options="closed:true,modal:true,title:'土地基础信息',collapsible:false,minimizable:false,maximizable:false,closable:true" style="width:960px;height:500px;">
		<div title="批复信息" data-options="
					tools:[{
						handler:function(){
							$('#groundhiregrid').datagrid('reload');
						}
					}]">
				
					<table id="groundhiregrid" style="overflow:auto"
	...
	<div id="groundbusinesswindow" class="easyui-window" data-options="closed:true,modal:true,title:'土地使用权转移',collapsible:false,minimizable:false,maximizable:false,closable:true,resizable:false" style="width:960px;height:500px;">
	</div>
</body>
</html>