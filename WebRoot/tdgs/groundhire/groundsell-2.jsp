		...
		if(opttype=='edit'){
			$.ajax({
				...
				success: function(jsondata){
					maxgroundarea = jsondata.landarea;
					$('#maxgroundarea').html('可使用土地面积'+maxgroundarea+'平方米');
					$('#belongtowns').combobox('setValue',jsondata.belongtowns);
					$('#detailaddress').val(jsondata.detailaddress);
					$('#locationtype').combobox('setValue',jsondata.locationtype);
				}
			});
			$.ajax({
				type: "post",
				url: "/GroundInfoServlet/getbusestateinfo.do",
				data: {busid:busid},
				dataType: "json",
				success: function(jsondata){
	...
	<form id="groundselldetailform" method="post">
			...
			<table id="tdjcxx" class="table table-bordered" >
				...
				<tr>
					...
					<td colspan="3">
						<input id="detailaddress" class="easyui-validatebox" style="width:400px;" disabled="true" type="text" name="detailaddress" data-options="required:true"></input>
					</td>
				</tr>
				<tr>
					<td align="right">
					...
					<td>
						<input id="transmoney" class="easyui-numberbox" style="width:200px;text-align:right" type="text" name="transmoney" data-options="min:0,required:true" precision="2" value="0.00" style="text-align:right" /></input>
					</td>
					<td align="right">
					土地面积（平方米）：
					</td>
