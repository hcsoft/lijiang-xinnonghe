...
<head>
	...
	function addOper(value,row,index){
		...
		if(row.checkstate == 0){   //完全匹配
			...
			result = '<a href="javascript:openCommitCheck(\''+row.serialno+'\')")>审核</a>';
	...
	function openCommitCheck(serialno){
	...
	function saveAndCommit(){
		//String serialno,String advice,String estatestate
		var params = {
	...
	function commitAdivice(params){
		$.ajax(
				   {
					   type:'post',
					   url:'/newdataarrange/estate.do',
					   data:params,
					   async:true,
					   dataType:'json',
					   success:function(jsondata){
						   if(jsondata.sucess){
							   $.messager.alert('提示消息','提交审核意见成功！','info',function(){
							      var newparams = getCondition();
								  $('#landdataarrangegrid').datagrid('reload',newparams);
							   });
						   }else{
							   $.messager.alert('提示消息','提交审核意见失败！','error');
						   }
	...
	function beginCheck(serialno){
		...
		$.messager.confirm('数据核查','你确定开始进行此条数据的核查吗？',function(r){
   			if(r){
   				$.ajax(
				   {
					   type:'post',
	...
	function addViewOper(value,row,index){
		var result = "";
		result = '<a href="javascript:viewCheckEstateInfo(\''+row.serialno+'\')")>核查、审核意见</a>&nbsp;&nbsp <a href="javascript:getMatchInfo(\''+row.serialno+'\')")>匹配情况</a>';
		return result;
	}
...
<body>
	    ...
	    <div data-options="region:'center'" style="height:500px;overflow: visible;" id="sourceDiv">
	     <form id="landdataarrangeform" method="post">
			<table id='landdataarrangegrid'  style="height:500px;overflow: scroll;">
				<thead>
					<tr>
						<th data-options="field:'serialno',hidden:true,width:100,align:'center',editor:{type:'validatebox'}">主键</th>
						<th data-options="field:'oper',formatter:addOper,hidden:false,width:100,align:'center',editor:{type:'validatebox'}">核查操作</th>
						<th data-options="field:'oper1',formatter:addViewOper,hidden:false,width:200,align:'left',editor:{type:'validatebox'}">查看操作</th>
						<th data-options="field:'comparestatename',width:100,align:'left',editor:{type:'validatebox'}">比对状态</th>
						<th data-options="field:'checkstatename',width:100,align:'left',editor:{type:'validatebox'}">核查状态</th>
						...
						<th data-options="field:'holddate',width:80,align:'center',formatter:formatterDate,editor:{type:'validatebox'}">土地获得日期</th>
						<th data-options="field:'area',width:120,align:'center',editor:{type:'validatebox'}">土地面积</th>
						<th data-options="field:'address',width:220,align:'center',editor:{type:'validatebox'}">详细地址</th>
						<th data-options="field:'landcertificate',width:120,width:120,align:'center',editor:{type:'validatebox'}">土地证号</th>
						<th data-options="field:'estateserial',hidden:true,align:'center',editor:{type:'validatebox'}">宗地编号</th>
						<th data-options="field:'getmoney',hidden:true,width:100,align:'center',editor:{type:'validatebox'}">成交价</th>
						<th data-options="field:'telephone',width:100,align:'center',editor:{type:'validatebox'}">联系电话</th>
						<th data-options="field:'importsourcename',width:80,align:'left',editor:{type:'validatebox'}">数据来源</th>
					</tr>
				</thead>
			</table>
	    </form>
	    ...
	    <div data-options="region:'west'" id="mainWestDiv" style="height:500px;width:160px;overflow: hidden;">
	        <div style="height:500px;background-color: white;width:100%">
	              <table id="statustable" width="100%"  cellpadding="3" cellspacing="0" border="1" bordercolor="#FCD209" style="border-collapse:collapse">
			       ...
			       <tr>
			           <td><input type="checkbox" data-type="x" value=" comparestate='1' "/>&nbsp;无采集土地数据</td>
			       </tr>
			       <tr>
			           <td><input type="checkbox" data-type="x" value=" comparestate='2' "/>&nbsp;完全匹配</td>
			       </tr>
			       <tr>
			           <td><input type="checkbox" data-type="x" value=" comparestate='3' "/>&nbsp;未匹配</td>
			       </tr>
			       <tr height="25px">
			           <td colspan="4">
		...
		<div id="tb" data-options="region:'north'" style="height:25px;width:100%;overflow: visible">
			<div style="height:25px;">
				...
				<span id="matchspan" style="display: none;">  
				               ...
				               完全匹配：<span id="matchNum" name="matchNum" style="color: red;font-weight: bold;font-size: 50;"></span>
				               无采集土地数据：<span id="notmatchNum" name="partmatchNum" style="color: red;font-weight: bold;font-size: 50;"></span> 
				               未匹配：<span id="partmatchNum" name="notmatchNum" style="color: red;font-weight: bold;font-size: 50;"></span>
				</span>
			</div>
		</div>
	</div>
	...
	style="width:640px;height:370px;">
	    <table id="advicetable" width="100%"  cellpadding="3" cellspacing="0" border="1" bordercolor="#FCD209" style="border-collapse:collapse">
		 ...
		 <div style="text-align:center;padding:5px;">  
			   <a id = 'compareHref' href="#" class="easyui-linkbutton" data-options="iconCls:'icon-save'" onclick="saveAndCommit()">审核通过</a>
		       <a id = 'compareHref' href="#" class="easyui-linkbutton" data-options="iconCls:'icon-save'" onclick="saveAndNotCommit()">审核不通过</a>
		 </div>
	</div>
	...
	<div id="matchinfowindow" class="easyui-window" data-options="border:false,closed:true,modal:true,title:'匹配情况',collapsible:false,minimizable:false,maximizable:false,closable:true" 
	style="width:640px;height:370px;">
			   ...
			   <td>
				    ...
				    <thead>
					  <tr>
						...
						<th data-options="field:'sourcedetailaddress',width:220,align:'left',editor:{type:'validatebox'}">详细地址</th>
						<th data-options="field:'sourcelandcertificate',width:120,align:'left',editor:{type:'validatebox'}">土地证号</th>
						<th data-options="field:'sourcearea',width:120,align:'left',editor:{type:'validatebox'}">面积</th>
						<th data-options="field:'sourceprice ',width:120,align:'left',editor:{type:'validatebox'}">地价</th>
			   ...
			   <td>
				    ...
				    <thead>
					  <tr>
						...
						<th data-options="field:'destdetailaddress',width:220,align:'left',editor:{type:'validatebox'},styler:destgridcolstyle">详细地址</th>
						<th data-options="field:'destlandcertificate',width:120,align:'left',editor:{type:'validatebox'},styler:destgridcolstyle">土地证号</th>
						<th data-options="field:'destarea',width:100,align:'left',editor:{type:'validatebox'},styler:destgridcolstyle">面积</th>
						<th data-options="field:'destprice ',width:100,align:'left',editor:{type:'validatebox'},styler:destgridcolstyle">地价</th>
