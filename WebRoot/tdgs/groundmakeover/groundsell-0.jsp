...
<script>
	...
	function savetdxx(){
			...
			if(!$('#limitbegins').datebox('isValid')){
				alert('使用年限起校验有误！');
				$('.datebox :text')[4].focus();
				return;
			}
			if(!$('#limitends').datebox('isValid')){
				alert('使用年限止校验有误！');
				$('.datebox :text')[5].focus();
				return;
			}
	...
	<form id="groundsellform" method="post">
			...
			<table id="tdjcxx" class="table table-bordered" >
				<tr>
					...
					<td align="left">
						<input id="landcertificate" class="easyui-validatebox" type="text" name="landcertificate" style="width:200px;" data-options="required:true"></input>
					</td>
				...
				<tr>
					...
					<td>
						<input id="landcertificatedates" name='landcertificatedates' class="easyui-datebox" style="width:200px;" data-options="validType:['datecheck'],required:true" ></input>
					</td>
					<td align="right">
						交付土地时间：
					</td>
