...
<script>
	...
	$(function(){
		...
		$.ajax({
			...
			$('#groundtransfergrid').datagrid({
				...
				toolbar:[{
					...
					handler:function(){
						...
						if(row){
							if(row.landstoreid == null){
								...
								belongtocountrycode = row.belongtowns;
					...
					handler:function(){
						var row = $('#groundtransfergrid').datagrid('getSelected');
						if(row){
							if(row.landstoreid == null && row.state != '4'){
								combobusinesstype='3';
								businesstype='31';
								maxgroundarea = row.landarea;
								locationtype = row.locationtype;
			...
			if(buttonbusinesstype=='0'){
				$('#cr').show();
				$('#wpxz').hide();
				$('#zr').hide();
				$('#cz').hide();
			}
			if(buttonbusinesstype=='1'){
				$('#cr').hide();
				$('#wpxz').show();
				$('#zr').hide();
				$('#cz').hide();
			}
			if(buttonbusinesstype=='2'){
				$('#cr').hide();
				$('#wpxz').hide();
				$('#zr').show();
				$('#cz').hide();
			}
			if(buttonbusinesstype=='3'){
				$('#cr').hide();
				$('#wpxz').hide();
				$('#zr').hide();
				$('#cz').show();
	...
	<div id="groundtransferquerywindow" class="easyui-window" data-options="closed:true,modal:true,title:'土地收储批复查询',collapsible:false,minimizable:false,maximizable:false,closable:true" style="width:940px;height:300px;">
		<div class="easyui-panel" title="" style="width:900px;">
			<form id="groundtransferqueryform" method="post">
				...
				<input type="hidden" name="state" id="state" value='2'/>
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="belongtocountrycode" id="belongtocountrycode" editable='false' data-options="
									valueField:'key',
									textField: 'value',
					...
					<tr>
						...
						<td><input id="querytaxpayerid" class="easyui-validatebox" type="text" name="querytaxpayerid"/></td>
			...
			<div style="text-align:center;padding:1px;">  
					<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-search'" onclick="groundquery()">查询</a>
			</div>
		</div>
	</div>
