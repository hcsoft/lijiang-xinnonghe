...
<head>
    ...
    function printPrepare(pt){
		...
		if(row == null || row.taxpayerid == '总计'){
			 if(pt === 1){
				 $.messager.alert('提示消息','请选择需要打印税务事项通知书的记录!','info');
			 }else if(pt === 0){
		...
		printtype = pt;
		var no = row.noticeno;
		var paynum = row.noticedaynum ? row.noticedaynum : '15';
    ...
    function processPrintInfo(){
    	...
    	$.ajax({
				  ...
				  async:true,
				  cache:false,
				  url: "/taxnoticecontrol/getprintinfo.do?d="+new Date(),
				  ...
				  success:function(jsondata){
					...
					for(var i = 0;i < details.length;i++){
						...
						if(detail.taxcode == '20')
							 continue;
						begintempinfo = '';
						endtempinfo = '';
						var beginDateObj = CommonUtils.getDate(detail.begindate);
						var endDateObj = CommonUtils.getDate(detail.enddate);
					    begintempinfo += beginDateObj.toString();
					    begintempinfo += '至'+endDateObj.toString();
					    begintempinfo += '所属期间应缴纳的'+detail.taxname;
					    endtempinfo = begintempinfo+detail.owetaxmoney+'元';
					    
					    begininfo = begininfo+begintempinfo+"，";
					    endinfo = endinfo+endtempinfo+'，';
					    totalvalue += detail.owetaxmoney;
					}
					$('#begininfo').text(begininfo);
					$('#endinfo').text(endinfo);
					...
					//$('#paydatelimit').text(converToChineseDate(new Date()));
			    	...
			    	LODOP.PRINT_INIT("晋宁县地方税务局税务事项通知书");
			    	...
			    	$.messager.confirm('打印提示','是否打印成功?',function(r){
			    		...
			    		if(r){
			    			 $.ajax({
							  ...
							  async:true,
							  cache:false,
							  url: "/taxnoticecontrol/printinfo.do?d="+new Date(),
							  data: params,
							  dataType: "text",
							  success:function(jsondata){
								  var params = WidgetUtils.showCommonQueryCondition();
								  params['pagesize'] = 15;
								   var opts = $('#owetaxgrid').datagrid('options');
								   opts.url = '/taxnoticecontrol/selecttaxnoticeinfo.do?d='+new Date();
    ...
    function processBackPrintInfo(){
    	...
    	$.ajax({
				  ...
				  async:true,
				  cache:false,
				  url: "/taxnoticecontrol/getbackprintinfo.do?d="+new Date(),
				  data: {'taxpayerid':row.taxpayerid},
				  dataType: "json",
				  success:function(jsondata){
					var printnumber = $('#printpreparewindow #printnumber').val();
	...
	$(function(){
		...
		function viewDetail(){
			...
			if(row == null || row.taxpayerid == '总计'){
			 ...
			 opts.url = '/taxnoticecontrol/selectdetailinfo.do?d='+new Date();
		...
		function printDetail(){
			var row = $('#owetaxgrid').datagrid('getSelected');
			if(row == null || row.taxpayerid == '总计'){
				 $.messager.alert('提示消息','请选择需要打印明细的记录!','info');
				 return;
			}
			 var params = {};
			 params['taxpayerid'] = row.taxpayerid;
			...
			$.ajax({
				  ...
				  async:true,
				  cache:false,
				  url: "/taxnoticecontrol/selectdetailinfo.do?d="+new Date(),
				  data: params,
				  dataType: "json",
				  success:function(jsondata){
					  jsondata = jsondata.rows;
					  $('#printdetailtable tbody tr').remove();
					  for(var i = 0;i < jsondata.length;i++){
						  var obj = jsondata[i];
						  var begindate = formatterDate(obj.taxdatebegin);
						  var enddate = formatterDate(obj.taxdateend);
						  var paydate = formatterDate(obj.paydate);
						  var td = "<td align='center'>"+obj.taxpayerid+"</td><td align='center'>"+obj.taxpayername+"</td>"+
						           "<td align='center'>"+obj.taxtypename+"</td><td align='center'>"+obj.taxname+"</td>"+
						           "<td align='center'>"+obj.levydatedesc+"</td>"+
						           "<td align='center'>"+begindate+"</td><td align='center'>"+enddate+"</td>"+
						           "<td align='center'>"+obj.owetaxmoney+"</td><td align='center'>"+paydate+"</td>";
						  var oTr = $('<tr>');
						  var oTd = $(td);
						  oTr.append(oTd);
						  $('#printdetailtable').append(oTr);
					  }
					  var LODOP=getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'));
					  LODOP.SET_LICENSES("昆明恒辰科技有限公司","055595969718688748719056235623","","");
				      LODOP.PRINT_INIT("欠税明细表格");
				      LODOP.SET_PRINT_PAGESIZE(2, 0, 0,"A4");
				      LODOP.SET_SHOW_MODE('LANDSCAPE_DEFROTATED',1);
					  LODOP.ADD_PRINT_TABLE(70,40,'94%','86%',document.getElementById("printdetaildiv").innerHTML);
				      LODOP.PREVIEW();
				  }
			   });
		}
		...
		function exportExcel(){
			...
			params['propNames'] = 'taxpayerid,taxpayername,landtaxamount,landtaxamountactual,landowetaxmoney,housetaxamount,'+
			               'housetaxamountactual,houseowetaxmoney,noticeno,noticedaynum,noticeprintnum,paydate';
			params['colNames'] = '计算机编码,纳税人名称,应缴土地使用税,已缴土地使用税,应补土地使用税,应缴房产税,已缴房产税,应补房产税,'+
                                 '税务事项通知书号,限期缴纳天数,打印次数,限缴日期';
			params['modelName']="税务事项通知书查询结果";
			CommonUtils.downloadFile("/taxnoticecontrol/taxnoticeexport.do?date="+new Date(),params);
		}
...
<body>
     <div class="easyui-layout" style="width:100%;height:570px;" id="layoutDiv" >
	     <div data-options="region:'center'">
			...
			<table id='owetaxgrid' class="easyui-datagrid" style="width:99;height:543px;overflow: scroll;"
				...
				<thead>
						...
						<th data-options="field:'noticeno',width:160,align:'left',editor:{type:'validatebox'}">税务事项通知书号</th>
						<th data-options="field:'noticedaynum',width:80,align:'center',editor:{type:'validatebox'}">限期缴纳天数</th>
						<th data-options="field:'noticeprintnum',width:60,align:'center',editor:{type:'validatebox'}">打印次数</th>
		...
		<div data-options="region:'north'" style="height:25px;width:100%;overflow: visible">
			<div style="height:25px;">
				...
				<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-print',plain:true" onclick="printDetail()">打印欠税明细</a>
				<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-print',plain:true" onclick="printPrepare(1)">税务事项通知书打印</a>
    ...
    <div id="printdetaildiv" style="display:none;letter-spacing:2px;" >
        <table id="printdetailtable" border=1 width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse" bordercolor="#000000">
           <caption style="font-weight: bold;font-size: 16pt;padding-bottom:5px;">欠税明细表格</caption>
            <thead>
             <tr>
				<td align="center">计算机编码</td>
				<td align="center">纳税人名称</td>
				<td align="center">税种</td>
				<td align="center">税目</td>
				<td align="center">征期类型</td>
				<td align="center">所属期起</td>
				<td align="center">所属期止</td>
				<td align="center">欠税</td>
				<td align="center">催缴日期</td>
			   </tr>
            </thead>
        </table>
    </div>
    ...
    <div id="printinfowindow" style="display:none;">
	     <div style="letter-spacing:2px;">
	        ...
	        <div align="center" style="font-size:26pt;font-weight:bold;font-family:宋体;line-height: 45px;">税务事项通知书</div>
	        <div align="center" style="font-size:16pt;font-family:仿宋;line-height: 35px;">晋地税<span id="taxdeptnum"></span>通<span id="printnumber" style="color:red"></span>号</div>
	        <div align="left" style="font-size:16pt;font-family:仿宋;line-height: 37px;color: red;">
	                            纳税人名称：<span id="taxpayername"></span><br/>
	                            纳税人识别号：<span id="taxcerno"></span>
	        </div>
	        <div style="font-size:16pt;font-family:仿宋;text-indent: 45px;line-height:150%;">
	                         你（单位）<span id="begininfo"></span>没有在规定的纳税期限缴纳。依据《中华人民共和国税收征收管理法》第三十一条规定：限你（单位）在<span id="paydatelimit"></span>前到<span id="taxlevname"></span>缴纳，<span id="endinfo"></span>合计<span id="totalvalue"></span>元。逾期不缴纳，将按《中华人民共和国税收征收管理法》第六十八条规定处理。
	        </div>
	...
	<div id="querydiv" class="easyui-window"  data-options="closed:true,modal:true,title:'查询条件',collapsible:false,
	minimizable:false,maximizable:false,closable:true" style="width:760px;height:200px;">
		<div id="querypanel" class="easyui-panel" style="width:740px">
