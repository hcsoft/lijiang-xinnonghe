...
<script>
					...
					handler:function(){
						...
						if(endEditing()){
							...
							if (data.length) {
								...
								var baseinfo = {"lessorid":tdxxrow.taxpayerid,"lessortaxpayname":tdxxrow.taxpayername,"estateid":estateid,"hiretype":0};
		...
		function refreshHire(){
			...
			$.ajax({
			   ...
			   data: {uuid:estateid,hiretype:0},//'ff171602b950f4ca5123c5c9edadc9d4'
			   dataType: "json",
			   success: function(jsondata){
				   //alert(JSON.stringify(jsondata));
				  $('#tdczfxx').datagrid('loadData',jsondata);
			   }
			});
			//}
		}
...
function bindevent(){
	...
	$(ed.target).bind('keyup', function(){
		...
		//	   type: "post",
		//	   async:false,
		//	   url: "/GroundInfoServlet/getreginfo.do",
...
$.extend($.fn.validatebox.defaults.rules, {   
		...
		<form id="tdczfxxform" method="post">
			<div title="土地承租方信息" data-options="
				tools:[{
					handler:function(){
						$('#tdczfxx').datagrid('reload');
					}
				}]">
			<table id="tdczfxx" style="width:1200px;height:380px"
			data-options="singleSelect:true,idField:'itemid'" rownumbers="true"> 
				<thead>
					<tr>
						...
						<th data-options="field:'agreementnumber',width:160,align:'center',editor:{type:'validatebox',options:{required:false}}">承租方文书号或协议号</th>
						<th data-options="field:'lesseesid',align:'center',hidden:true,editor:{type:'validatebox'}"></th>
						<th data-options="field:'lesseestaxpayername',width:120,align:'center',editor:{type:'validatebox',options:{required:true}}">承租方名称(回车查询)</th><!-- 待定 -->
						...
						<th data-options="field:'landarea',width:80,align:'center',editor:{type:'numberbox',options:{precision:2,min:0,required:true}}">承租面积</th>
						...
						<th data-options="field:'transbegindates',width:80,align:'center',editor:{type:'datebox',options:{required:true,validType:['datecheck']}}">承租时间起</th>
						<th data-options="field:'transenddates',width:80,align:'center',editor:{type:'datebox',options:{required:true,validType:['datecheck']}}">承租时间止</th>
					</tr>
				</thead>
			</table>
		</div>
